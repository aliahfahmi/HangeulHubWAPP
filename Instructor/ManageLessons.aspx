...
<form id="form1" runat="server">
    ...
    <main class="container">
        ...
        <div class="lesson-grid" id="lessonGrid">
            <asp:Repeater ID="rptLessons" runat="server" OnItemCommand="rptLessons_ItemCommand">
                <ItemTemplate>
                    <!-- FIX: level/course display now comes from the joined courseTable
                         columns (courseLevel, courseTitle) instead of a GetLevelName()
                         lookup that hardcoded courseID -> level text. -->
                    <div class="lesson-card"
                         data-level='<%# GetLevelKey(Eval("courseLevel")) %>'
                         data-lessonid='<%#: Eval("lessonID") %>'
                         data-title='<%#: GetLessonTitle(Eval("type")) %>'
                         data-course='<%#: Eval("courseTitle") %>'>
                        ...
                        <div class="lesson-body">
                            <span class='badge <%# GetLevelKey(Eval("courseLevel")) %>'>
                                <%# GetLevelDot(Eval("courseLevel")) %> <%#: Eval("courseLevel") %>
                            </span>
    ...
    <div class="modal-overlay" id="lessonModal">
        <div class="modal">
            ...
            <!-- FIX: was 3 hardcoded <asp:ListItem> (C001/C002/C003). Now bound from
                 courseTable in code-behind (LoadCourseDropdown), so it stays correct
                 even if courses are added, renamed, or reordered. -->
